...
evalCBN ENat0 = ENat0
evalCBN (EIfElse e1 e2 e3 e4) =
    if (evalCBN e1) == (evalCBN e2) then
        evalCBN e3
    else
        evalCBN e4
evalCBN x = x
...
subst id s (EIfElse e1 e2 e3 e4) = EIfElse (subst id s e1) (subst id s e2) (subst id s e3) (subst id s e4)
